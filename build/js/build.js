...
(function(root, factory) {
	...
	function filter(params) {
		var defaultOptions = {
			...
			filters: '.filter',
			listElementSelector: '.filterobj',
			triggerSelector: '.my-trigger',
			triggerAllSelector: '.all-btn',
			sortSelector: '.sort-button',
			numberOfColumns: 4			
		},
		...
		filterobjects = [],
		filters = Array.prototype.slice.call(document.querySelectorAll(_opts.filters)),
		filtertriggers = Array.prototype.slice.call(document.querySelectorAll(_opts.triggerSelector)),
		filtertriggerAll = Array.prototype.slice.call(document.querySelectorAll(_opts.triggerAllSelector)),
		activeClass = _opts.activeClass || 'active',
		filtertypes = [],
		filterdata = [],
		currentlyShown = filterdata,
		currentfilter = {},
		sortbytriggers = Array.prototype.slice.call(document.querySelectorAll(_opts.sortSelector)),
		sortingdata = [];
		...
		function init() {
			...
			filters.forEach(function(filter) {
				var type = filter.getAttribute('data-filtertype');
				filtertypes.push(type);
				currentfilter[type] = [];
			});

			sortbytriggers.forEach(function(sort) {
				var sortBy = sort.getAttribute('data-sortby');
				sortingdata.push(sortBy);
			});

			//Listen to clickevents on sortbytriggers
			sortbytriggers.forEach(function(trigger) {
				trigger.addEventListener('click', SetCurrentSorting);
			});

			//Add activeclass to All-triggers
			filtertriggerAll.forEach(function(trigger) {
				trigger.classList.add(activeClass);
			});

			//Listen to clickevents on filtertriggers
			filtertriggers.forEach(function(trigger) {
				trigger.addEventListener('click', SetCurrentFilter);
			});
		...
		function SetFilterData() {
			...
			filterobjects = Array.prototype.slice.call(document.querySelectorAll(_opts.listElementSelector));
			...
			filterobjects.forEach(function(object, i) {
				...
				filtertypes.forEach(function(filtertype) {
					var datafilter = object.getAttribute('data-' + filtertype).replace(/ /g,'')
					filterdata[i].filters[filtertype] = datafilter.split(',');
				});

				sortingdata.forEach(function(sortBy) {
					var datasorting = object.getAttribute('data-' + sortBy).replace(/ /g,'')
					filterdata[i].sorting[sortBy] = datasorting;
				});
		...
		function SetCurrentFilter(e) {

			var elem = e.currentTarget,
			parent = elem.parentNode,
			filtertype = elem.parentNode.getAttribute('data-filtertype'),
			filter = elem.getAttribute('data-filtervalue') !== null ? elem.getAttribute('data-filtervalue').replace(/ /g,'') : null;
			...
			if (currentfilter[filtertype].indexOf(filter) > -1) {
				currentfilter[filtertype].splice(currentfilter[filtertype].indexOf(filter), 1);
				elem.classList.remove(activeClass);
			} else {
				currentfilter[filtertype].push(filter);
				elem.classList.add(activeClass);
			}

			//Show all elements if the All-filter is triggered or if filter is empty
			if (elem.classList.contains(_opts.triggerAllSelector.substring(1, _opts.triggerAllSelector.length)) || !currentfilter[filtertype].length > 0) {
				currentfilter[filtertype] = [];
				[].forEach.call(parent.querySelectorAll(_opts.triggerSelector), function(trigger){
					trigger.classList.remove(activeClass);
				});
				parent.querySelector(_opts.triggerAllSelector).classList.add(activeClass);
				UpdateFilter(filtertype);
				return;
			}

			parent.querySelector(_opts.triggerAllSelector).classList.remove(activeClass);
			UpdateFilter(filtertype);
		...
		function UpdateFilter() {
			...
			var filterOnType = function(object) {
				var showobject = false;
				if (currentfilter[type].length > 0) {
					currentfilter[type].forEach(function(value) {
						if (object.filters[type].indexOf(value) > -1) {
							showobject = true;
						} else {
							if(listitemsToHide.indexOf(object) === -1) {
								listitemsToHide.push(object);
							}
						}
					});
				} else {
			...
			listitemsIndexToShow = [];

			//Iterate over the currentfilterobject
			Object.keys(currentfilter).forEach(function(key) {
				type = key;
				//add objects that match the currentfilter
				listitems = listitems.filter(filterOnType);
			});
			
			//Hide other objects
			listitemsToHide.forEach(function(object) {

				if(listitems.indexOf(object) > -1) return;

				if(_opts.animation) {
					transformHide(object.el);
				} else {
					object.el.style.display = 'none';
				}

			});

			//Show the objects that been filtered
			listitems.forEach(function(object, index) {

				if(_opts.animation) {
					transformShow(object.el, object.order, index);
				} else {
					object.el.style.display = 'block';
				}

			});

			// Array we use when sorting data
			currentlyShown = listitems;

		};

		function SetCurrentSorting(e) {
			...
			if(elem.classList.contains('active')) {
				...
				sortvalue = elem.getAttribute('data-sortby') !== null ? elem.getAttribute('data-sortby').replace(/ /g,'') : null;
				sorttype = elem.getAttribute('data-sorttype') !== null ? elem.getAttribute('data-sorttype').replace(/ /g,'') : null;
			}

			UpdateSortOrder(sortvalue, sorttype);
		};

		function UpdateSortOrder(value, type) {

			var listitems = currentlyShown;

			listitems.sort(function(a, b){

				if(value !== 'default') {
					return type === 'int' ? b.sorting[value] - a.sorting[value] : a.sorting[value].localeCompare(b.sorting[value]);
				} else {
			...
			//Show the objects in new order
			listitems.forEach(function(object, index) {
				transformShow(object.el, object.order, index);
			});
		};

		function transformShow(elem, totalIndex, index) {

			listitemsIndexToShow.push(index);

			var currentRow = (totalIndex - (totalIndex % _opts.numberOfColumns)) / _opts.numberOfColumns,
				currentColumn = totalIndex - (currentRow * _opts.numberOfColumns),
				targetRow = (index - (index % _opts.numberOfColumns)) / _opts.numberOfColumns,
				targetColumn = (listitemsIndexToShow[listitemsIndexToShow.length - 1] - (targetRow * _opts.numberOfColumns)),
				goToColumn = (targetColumn * self.listElementWidth) - (currentColumn * self.listElementWidth),
				goToRow = (targetRow * self.listElementHeight) - (currentRow * self.listElementHeight);
			
			Velocity(elem.firstElementChild, 
				animation(goToColumn, goToRow, 1, 1, 0, 0, 1), 
				{ 
					duration: _opts.animationSettings.show.duration, 
					easing: _opts.animationSettings.show.easing, delay: Math.floor((Math.random() * _opts.animationSettings.show.delay) + 0) 
				}
			);

		};

		function transformHide(elem) {

			var rotateY = Math.floor(Math.random()*20) + 0; 
			rotateY *= Math.floor(Math.random()*2) == 1 ? 1 : -1;

			var rotateX = Math.floor((Math.random() * 15) + 30);

			Velocity(elem.firstElementChild, 
				animation(0, 0, 0.4, 0.4, rotateX, rotateY, 0), 
				{ 
					duration: _opts.animationSettings.hide.duration, 
					easing: _opts.animationSettings.hide.easing, delay: Math.floor((Math.random() * _opts.animationSettings.hide.delay) + 0) 
				}
			);

		};

		function animation(tX, tY, sX, sY, rX, rY, o) {
			return {
				translateX: tX + 'px',
				translateY:  tY + 'px',
				scaleX: sX,
		...
		function getOffsets() {
			listElementWidth = getOuterWidth(filterobjects[0]);
			listElementHeight = getOuterHeight(filterobjects[0]);
			listElementWidthNoMargin = filterobjects[0].offsetWidth;
			listElementHeightNoMargin = filterobjects[0].offsetWidth;
		};
...
},{"./modules/helpers":4,"mono-filter":1}],4:[function(require,module,exports){
var Helpers = {
